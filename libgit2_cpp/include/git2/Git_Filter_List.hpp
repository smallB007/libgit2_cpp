...
public:
	...
	shared_ptr_t<Git_Buf> apply_to_file(const string_t&)const;
	bool contains(const string_t& name)const;
	size_t length()const;
	shared_ptr_t<Git_Filter_List> load(const Git_Blob& blob, const string_t& relative_path, git_filter_mode_t filterMode, uint32_t flags) const;
	shared_ptr_t<Git_Filter> lookup(const string_t&)const;
	void register_filter(const string_t&,Git_Filter&, int priority)const;
	...
	string_t filter_source_path(const Git_Filter_Source& src)const;
	shared_ptr_t<Git_Repo> filter_source_repo(const Git_Filter_Source& src)const;
	void unregister(const string_t&)const;
