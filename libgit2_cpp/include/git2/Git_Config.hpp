...
private:
	...
	string_t find_path_helper_()const;
	template<class T>
...
LIBGIT2_CONFIG_INTERFACE
	...
	void add_file_on_disk(const string_t& path, const Git_Config_Level& config_level = GIT_CONFIG_HIGHEST_LEVEL, bool force = false);
	void delete_entry(const string_t& name);
	string_t find_global_path()const;
	string_t find_program_data_path()const;
	string_t find_system_path()const;
	...
	string_t get_path(const string_t & varName)const;
	...
	shared_ptr_t<Git_Config> open_on_disk(const string_t&)const;
	bool parse_bool(const string_t& bool_val)const;
	
	template<class T>
	T parse_int(const string_t&)const;

	string_t parse_path(const string_t&)const;
