...
public:
	...
	shared_ptr_t<Git_Buf> apply_to_file(const file_path_t&)const;
	bool contains(const string_t& name)const;
	size_t length()const;
	shared_ptr_t<Git_Filter_List> load(const Git_Blob& blob, const file_path_t& relative_path, git_filter_mode_t filterMode, uint32_t flags) const;
	...
	file_path_t filter_source_path(const Git_Filter_Source& src)const;
